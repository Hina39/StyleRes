...
def process_image(image, method, edit, factor, is_align_checked):
    ...
    image = image_processor.postprocess_image(
        image.detach().cpu().numpy(), is_batch=False
    )
    return image
...
def update_slider(method):
    ...
    return gr.Slider.update(
        ...
        value=0,
        step=step,
        label=f"Strength [{minimum}, {maximum}]",
    )
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        image_input = gr.Image(
            ...
            shape=(256, 256),
            label="Input Image",
            value="samples/demo_samples/116.jpg",
        ).style(height=256)
        image_output = gr.Image(
            type="pil", shape=(256, 256), label="Output Image"
        ).style(height=256)

    with gr.Row():
        with gr.Column(scale=0.25, min_width=50):
            methods_drowdown = gr.Dropdown(
                methods, label="Choose Method", value=methods[0]
            )
        with gr.Column(scale=0.25, min_width=50):
            edits_dropdown = gr.Dropdown(
                utils.get_edits(methods[0]),
                label="Choose Edit",
                value=utils.get_edits(methods[0])[0],
            )
    ...
    methods_drowdown.change(
        update_edit_dropdown, inputs=methods_drowdown, outputs=edits_dropdown
    )
    methods_drowdown.change(
        update_slider, inputs=methods_drowdown, outputs=factor_slider
    )
    submit_button.click(
        process_image,
        inputs=[
