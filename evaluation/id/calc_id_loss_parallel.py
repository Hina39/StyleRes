...
from evaluation.id.mtcnn.mtcnn import MTCNN
from evaluation.id.encoders.model_irse import IR_101
from evaluation.id.configs.paths_config import model_paths
...
def parse_args():
    ...
    parser.add_argument(
        "--gt_path", type=str, default="gt_images", help="path to gt images"
    )
...
def run(args):
    run_on_step_output(args)


def run_on_step_output(args):
    file_paths = []
    for f in os.listdir(args.gt_path):
        image_path = os.path.join(args.output_path, f)
    ...
    result_str = "New Average score is {:.5f}+-{:.5f}".format(mean, std)
    print(result_str)
