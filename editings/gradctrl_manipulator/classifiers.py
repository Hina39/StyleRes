...
class MultiLabelClassifier(nn.Module):
    def __init__(
        ...
        for i in range(self.n_branches):
            if multiclass_classes is not None and self.multiclass_classes[i] > 1:
                classifier_dict[str(i)] = MulticlassClassifier(
                    hidden_size=hidden_size, n_classes=self.multiclass_classes[i]
                )
            else:
    ...
    def forward(self, x):
        ...
        shared = torch.cat(
            [self.shared(x[:, :, i]).unsqueeze(2) for i in range(self.n_branches)],
            dim=2,
        )
        out = []
        for i in range(self.n_branches):
