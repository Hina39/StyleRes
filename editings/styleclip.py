...
from editings.styleclip_directions.styleclip_global_calculator import (
    StyleCLIPGlobalDirection,
)
import torch
import csv
...
class StyleClip:
    def __init__(self) -> None:
        ...
        with open(
            os.path.join(Settings.styleclip_settings, "styleclip_mapping_configs.csv"),
            "r",
        ) as f:
            reader = csv.reader(f)
            for row in reader:
                key = row.pop(0)
                self.styleclip_mapping_configs[key] = list(
                    map(lambda x: True if x == "True" else False, row)
                )
    ...
    def edit(self, latent, cfg):
        with torch.no_grad():
            ...
            if cfg.type == "global":
                ...
                directions = calculator.get_delta_s(
                    cfg.neutral_text, cfg.target_text, cfg.disentanglement
                )
                edited_styles = []
    ...
    def build_global_direction_calculator(self):
        try:  # Check if loaded
            global_direction_calculator = getattr(self, "global_direction_calculator")
        except:
            delta_i_c = (
                torch.from_numpy(np.load(Settings.delta_i_c))
                .float()
                .to(Settings.device)
            )
            with open(Settings.s_statistics, "rb") as channels_statistics:
                ...
                s_std = [
                    torch.from_numpy(s_i).float().to(Settings.device) for s_i in s_std
                ]
            ...
            setattr(self, "global_direction_calculator", global_direction_calculator)
    ...
    def build_mapper(self, editname):
        try:  # Check if loaded
            ...
            ckpt = torch.load(
                os.path.join(Settings.styleclip_mapper_directions, f"{editname}.pt")
            )
