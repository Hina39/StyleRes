...
class StyleRes(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = E4E_Inversion(
            resolution=256, num_layers=50, mode="ir_se", out_res=64
        )
    ...
    def edit_images(self, image, cfg):
        ...
        with torch.no_grad():
            ...
            input_is_stylespace = (
                True if cfg.method == "styleclip" and cfg.type == "global" else False
            )
        ...
        with torch.no_grad():
            ...
            skips["F_feats"] = self.generator(
                latents, skips, return_f=True, **self.G_kwargs_val
            )
