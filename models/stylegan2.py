...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(
        self, x, w, noise_mode="random", fused_modconv=True, gain=1, return_styles=False
    ):
        global latent_space
        if latent_space == "w+":
...
class FeatureAlignment(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.first_layer = nn.Conv2d(
            in_channels, t_channel, kernel_size=1, padding=0, bias=True
        )

        self.conv1 = nn.Sequential(*[ResLayers(t_channel, t_channel, 1)])
        self.conv2 = nn.Sequential(
            *[ResLayers(t_channel, t_channel, 2), ResLayers(t_channel, t_channel, 1)]
        )
        ...
        self.dconv1 = nn.Sequential(
            *[ResLayers(t_channel, t_channel, 1), ResLayers(t_channel, t_channel, 1)]
        )
        ...
        self.out_layer = nn.Conv2d(
            t_channel, out_channels, kernel_size=1, padding=0, bias=True
        )
    ...
    def forward(self, encoder_feats, generator_feats):
        ...
        shape = f3.shape[-1]
        df1 = F.interpolate(
            f3, size=(shape * 2, shape * 2), mode="bilinear", align_corners=True
        )
        df2 = self.dconv1(df1 + f2)
        df2 = F.interpolate(
            df2, size=(shape * 4, shape * 4), mode="bilinear", align_corners=True
        )
...
class FeatureExtraction(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = nn.Sequential(
            *[
                ResLayers(t_channel, t_channel, 1),
                ResLayers(t_channel, out_channels, 1),
                ResLayers(out_channels, out_channels, 1),
            ]
        )
...
class GateNetwork(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down1 = nn.Conv2d(
            in_channels, t_channel, kernel_size=3, padding=1, bias=True
        )
        ...
        self.convs = nn.Sequential(
            *[
                ResLayers(in_channels, in_channels, 1),
                ResLayers(in_channels, out_channels, 1),
                ResLayers(out_channels, out_channels, 1),
            ]
        )
    ...
    def __init__(
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if resolution == embed_res:
            ...
            # E1
            self.modify_feature_alignment = FeatureAlignment(
                in_channels=in_c, out_channels=512
            )
            self.modify_feature_extraction = FeatureExtraction(
        ...
        **layer_kwargs,
        ...
        if self.in_channels == 0:
            ...
            if x.shape[-1] == self.embed_res and highres_outs is not None:
                ...
                aligned_feats = self.modify_feature_alignment(
                    highres_outs[f"{self.embed_res}x{self.embed_res}"],
                    highres_outs["F_feats"],
                )
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(
        self, ws, highres_outs=None, return_f=False, return_styles=False, **block_kwargs
    ):
        block_ws = []
        global latent_space
        with torch.autograd.profiler.record_function("split_ws"):
            ...
            for res in self.block_resolutions:
                ...
                if latent_space == "w+":
                    ...
                    block_ws.append(
                        ws[s_idx : s_idx + block.num_conv + block.num_torgb]
                    )
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f"b{res}")
            x, img = block(
                x, img, cur_ws, highres_outs, return_f, return_styles, **block_kwargs
            )
